...
이 문서는 비개발자 직원도 `korea-talent-search-skills`를 설치하고 Claude Code에서 잡코리아/사람인 인재검색을 안전하게 수행할 수 있도록 안내한다.
...
Claude에게 요청할 때는 “이 직무에서는 어떤 근거를 높게 볼지”를 같이 적으면 결과 품질이 좋아진다.
...
4. Claude Code가 설치되어 있는지 확인한다.
5. 없으면 회사 표준 방식으로 Claude Code를 설치한다.
6. 채용 인재검색 스킬 2개를 한 번에 설치한다.
7. Claude Code를 실행한다.
8. Claude에게 잡코리아/사람인 검색 요청을 한다.
9. 브라우저가 열리면 직원 본인이 직접 로그인/2차 인증을 완료한다.
10. Claude가 마스킹된 정보 기준으로 후보 shortlist를 만든다.
...
PowerShell 또는 Terminal에서 확인한다.

```bash
claude --version
```

정상적으로 버전이 나오면 다음 단계로 진행한다.

설치되어 있지 않다면 회사 표준 Claude Code 설치 가이드를 따른다. 회사에서 별도 제한이 없다면 아래 방식으로 설치할 수 있다.
...
- Claude Code 로그인은 개인/회사 Anthropic 계정 정책에 따른다.

## 5. Claude/Codex에게 설치까지 맡기는 방법

Node.js, Git, Claude Code 설치가 익숙하지 않은 직원은 Claude Code, Codex, 또는 터미널을 다룰 수 있는 다른 LLM 에이전트에게 설치를 맡길 수 있다.
...
- Git이 없으면 설치
- Claude Code가 없으면 설치
- korea-talent-search-skills를 Claude Code용으로 설치
- 설치 후 두 스킬이 보이는지 확인
...
4. Claude Code가 없으면 아래 명령으로 설치해줘.
   npm install -g @anthropic-ai/claude-code
...
3. Claude Code가 없으면 아래 명령으로 설치해줘.
   npm install -g @anthropic-ai/claude-code
...
- Claude Code 로그인 또는 Anthropic 계정 인증이 필요함
```
...
```text
채용 인재검색용 Claude Code 스킬 설치를 위해 아래 프로그램/명령 실행 권한이 필요합니다.
...
Windows PowerShell, macOS Terminal 모두 아래 명령을 그대로 복사해 실행한다.
...
보이지 않으면 Claude Code를 종료 후 다시 실행하거나, 설치 명령을 다시 실행한다.
...
Claude Code가 실행되면 아래 예시 중 하나를 복사해 입력한다.
...
Claude가 여는 브라우저가 평소 사용하는 Chrome/Edge와 다른 프로필일 수 있다. 이 경우 사이트가 새 기기로 인식해 다시 로그인이나 2차 인증을 요구할 수 있다. 이는 정상이다.
...
2. Claude Code를 종료 후 다시 실행한다.
3. PowerShell을 닫았다가 다시 연다.
4. 회사 보안 프로그램이 자동 브라우저 실행을 막는다면 IT 담당자에게 문의한다.

### 로그인 상태가 공유되지 않는 경우

Claude가 연 브라우저가 기존 브라우저와 다른 프로필이면 쿠키가 공유되지 않는다.

해결:

```text
Claude가 연 브라우저 안에서 잡코리아/사람인 기업회원 로그인을 다시 한다.
```
...
`npx`, `node`, `npm`, `claude` 명령을 찾을 수 없으면 설치 후 PowerShell을 새로 열었는지 확인한다.
...
이렇게 말해도 브라우저 열기 자체를 못 하면 Claude Code 실행 환경의 브라우저 권한/도구 문제일 수 있다.
